...
//     if (browser) {
//     try {
//         const awards = await csv('/data/awards.csv', autoType);
//         const talks = await csv('/data/talks.csv', autoType);
...
export const load = (async ({ fetch, params }) => {
    ...
    const result = await fetch("/data/awards.csv", {
    ...
    // const result_ = await fetch("https://raw.githubusercontent.com/lnicoletti/personal-website/main/src/lib/data/talks.csv", {
        const result_ = await fetch("/data/talks.csv", {
      headers: { "content-type": "text/csv;charset=UTF-8" },
    });
